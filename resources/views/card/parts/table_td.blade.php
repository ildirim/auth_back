<td>{{ $card->card_no }}</td>
<td>{{ $statusEnum::getStatusList()[$card->status] }}</td>

<td class="text-end">
	<a href="#"  data-id="{{ $card->id }}" class="btn btn-icon btn-bg-light btn-edit btn-sm btn-edit-card" data-toggle="modal" data-target="#modal-edit-card">
	...
	<a href="javascript:void(0);" data-action="{{ route('deleteCard', $card->id) }}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-icon btn-bg-light btn-delete btn-sm btn-delete-card" data-id="{{ $card->id }}">
		<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
			width="24" height="24"
			viewBox="0 0 24 24"
			style=" fill:#000000;">
